...
class ASFF_Detect(nn.Module):  # add ASFFV5 layer and Rfb
    ...
    def forward(self, x):
        z = []  # inference output
        result = [self.l2_fusion(x), self.l1_fusion(x), self.l0_fusion(x)]
...
def parse_model(d, ch):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        if m in (Conv, GhostConv, Bottleneck, GhostBottleneck, SPP, SPPF, ASPP, DWConv, MixConv2d, Focus,
                 BottleneckCSP, CBAM, ResBlock_CBAM, SE, ECA, C3SE, C3ECA, C3CBAM, C3CA,SimSPPF,SimConv,
                 CoordAtt, CrossConv, C3, CTR3, Involution, C3SPP, C3Ghost, CARAFE, nn.ConvTranspose2d,
                 DWConvTranspose2d, C3x, SPPCSPC, GhostSPPCSPC, BottleneckCSPA, BottleneckCSPB, BottleneckCSPC,
            ...
            if m in [BottleneckCSP, C3, C3TR, CTR3, C3Ghost, C3x,
                     SPPCSPC, GhostSPPCSPC, C3SE, C3ECA, C3CBAM, C3CA,
            ...
            if isinstance(args[1], int):  # number of anchors
                args[1] = [list(range(args[1] * 2))] * len(f)
        elif m in [V7_Detect, IDetect, IAuxDetect, IBin]:
            args.append([ch[x] for x in f])
            if isinstance(args[1], int):  # number of anchors
                args[1] = [list(range(args[1] * 2))] * len(f)
        elif m is Contract:
