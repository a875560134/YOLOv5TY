        ...
        for i, pi in enumerate(p):  # layer index, layer predictions
            ...
            if n:
                ...
                if self.g2 > 0:  # Focal-EIOU https://arxiv.org/abs/2101.08158
                    lbox += ((bbox_iou(pbox.T, tbox[i]) ** self.g2) * (1 - iou)).mean()
                else:
...
class ComputeLossBinOTA:
    ...
    def __init__(self, model, autobalance=False):
        ...
        det = model.module.model[-1] if model.model[-1] else model.model[-1]  # Detect() module
        self.balance = {3: [4.0, 1.0, 0.4]}.get(det.nl, [4.0, 1.0, 0.25, 0.06, .02])  # P3-P7
        self.ssi = list(det.stride).index(16) if autobalance else 0  # stride 16 index
        self.BCEcls, self.BCEobj, self.gr, self.hyp, self.autobalance = BCEcls, BCEobj, model.gr, h, autobalance
...
def bbox_iouu(box1, box2, xywh=True, GIoU=False, DIoU=False, CIoU=True,EIoU=False,SIoU=False,a=3, eps=1e-7):
    ...
    if GIoU or DIoU or CIoU or EIoU or SIoU:
        ...
        ciou=v * alpha_ciou + eps
        aciou=torch.pow(ciou,a)
        #GIoU
        c_area = torch.max(cw * ch + eps, union) # convex area
        giou=(c_area - union) / c_area + eps
        agiou=torch.pow(giou, a)
        #SIoU
